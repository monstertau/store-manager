...
import CustomizedDialogs from "../bill/modal";
...
const columnProducts = [
  ...
  { column: "Supplier", row: "supplier_name", type: "text" },
...
function ConnectediProduct(props) {
  ...
  const handleChangeSearch = async (e) => {
    new Promise(async (resolve, reject) => {
      let result = await buyService.searchBuy(0, state.numberRowPerPage, {
        purpose: e,
        description: e,
      });
  ...
  const handleChangePage = async (page) => {
    new Promise(async (resolve, reject) => {
      let result = await buyService.searchBuy(
  ...
  const handleChangeRowPerPage = async (pageSize) => {
    props.alertClear();
    new Promise(async (resolve, reject) => {
      let result = await buyService.searchBuy(0, pageSize, state.search);
      if (result.success == false) {
        props.alertError(result.message);
        reject();
      } else {
  ...
  const defaultSearch = () => {
    new Promise(async (resolve, reject) => {
      var result = await buyService.searchBuy(
        state.pageNumber,
        state.numberRowPerPage,
        {}
      );
  ...
  const searchBuyDate = () => {
    new Promise(async (resolve, reject) => {
      var result = await buyService.searchBuy(0, state.numberRowPerPage, {
    ...
    <div style={{ padding: "10px 15px" }}>
      ...
      <div>
        <CustomizedDialogs
          ...
          rowData={state.rowData.buy_items}
          rowObj={state.rowData}
          title={"Details Imported Product"}
